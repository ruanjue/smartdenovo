...
void clp_high_err_region_wtclp(WTCLP *wt, uint32_t min_dep){
	pb_seq_t *pb;
	pb_aln_t *hit;
	u4v *brks;
	uint64_t i, idx;
	...
	for(i=0;i<wt->seqs->size;i++){
		...
		if(brks->size == 0){
			pb->clp_x = pb->clp_y = 0; pb->closed = 2;
		} else {
...
int usage(){
	...
	" -x <int>    For debug. 1: chimera checking; 2:conntection checking, [3]\n"
	);
	return 1;
}
...
int main(int argc, char **argv){
	...
	int win_size, min_crs_dep, debug_x;
	float min_sm;
	uint64_t tol;
	uint32_t nflt, nclp;
	ovls = init_cplist(4);
	...
	whole = 0;
	debug_x = 3;
	while((c = getopt(argc, argv, "hi:b:o:fFs:m:Cc:r:w:k:d:n:x:")) != -1){
		switch(c){
			case 'h': return usage();
	...
	if(whole){
		fprintf(stderr, "Orignal: legal overlaps = %llu\n", (unsigned long long)tol);
	} else {
		fprintf(stderr, "Before: legal overlaps = %llu\n", (unsigned long long)tol);
		clp_high_err_region_wtclp(wt, min_dep);
		tol = call_legal_overlaps_wtclp(wt);
		fprintf(stderr, "After:  legal overlaps = %llu\n", (unsigned long long)tol);
	}
	fprintf(stderr, "[%s] Done\n", date());
	fprintf(stderr, "[%s] init iteration\n", date());
