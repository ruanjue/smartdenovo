...
push(@lines, qq/EXE_ZMO=$opts{zmo}/, qq/EXE_GBO=$opts{gbo}/, qq/EXE_CLP=$opts{clp}/, qq/EXE_LAY=$opts{lay}/, qq/EXE_CNS=$opts{cns}/);
...
push(@lines, q/$(PREFIX).lay.utg $(PREFIX).lay:$(PREFIX).ovl.short $(PREFIX).gbo.short $(PREFIX).obt/);
